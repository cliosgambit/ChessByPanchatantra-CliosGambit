...
const { runPythonScript, STOCKFISH_EXE } = require('../utils/brilliancePython');

const scriptPath = path.join(__dirname, '..', 'brilliance_stage2.py');
const stockfishPath = STOCKFISH_EXE;
...
function runPythonStage2(pgn) {
  return runPythonScript(scriptPath, JSON.stringify({ pgn, engine_path: stockfishPath }));
}
