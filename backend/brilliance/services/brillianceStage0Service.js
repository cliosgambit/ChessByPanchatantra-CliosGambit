...
const { runPythonScript } = require('../utils/brilliancePython');
...
function parseFeatureRow(row) {
  ...
  return {
    ...
    net_see_value: features?.see?.net_see_value ?? row.see_value,
    net_see_applied: Boolean(features?.see?.net_see_applied),
    net_see_reason: features?.see?.net_see_reason ?? null,
    moving_piece_type: row.moving_piece_type ?? features?.see?.moving_piece_type ?? null,
    moving_piece_value: features?.see?.moving_piece_value ?? 0,
    moving_piece_landing_square: features?.see?.moving_piece_landing_square ?? null,
    captured_piece_type: features?.see?.captured_piece_type ?? null,
    captured_piece_square: features?.see?.captured_piece_square ?? null,
    captured_value: features?.see?.captured_value ?? 0,
    landing_post_move_see: features?.see?.landing_post_move_see ?? 0,
    landing_en_prise: Boolean(features?.see?.landing_en_prise),
    ...
    negative_see_sacrifice: Boolean(
      features?.see?.negative_see_sacrifice
    ),
    positional_risk: Boolean(features?.see?.positional_risk),
    newly_exposed_sacrifice: Boolean(features?.see?.newly_exposed_sacrifice),
    suppression_reason: features?.suppression_reason ?? features?.see?.suppression_reason ?? null,
    early_game_blocked: Boolean(features?.early_game_blocked),
...
function runPythonStage0(pgn) {
  return runPythonScript(scriptPath, JSON.stringify({ pgn }));
}
