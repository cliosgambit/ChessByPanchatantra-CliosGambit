...
function importCustomPgn(pgnText, { originalFilename = 'custom_game.pgn', lichessGameId = null } = {}) {
  const text = String(pgnText || '').trim();
  if (!text) throw new Error('Empty PGN');

  ensureUploadsDir();
  const parsed = parsePgnGame(text);
  if (lichessGameId) parsed.lichessGameId = lichessGameId;
...
function findGameByLichessGameId(lichessGameId) {
  const id = String(lichessGameId || '').trim();
  if (!id) return null;
  const row = db
    .prepare(
      `SELECT id FROM lichess_pgn_games
       WHERE lichess_game_id = ? AND status = 'cleaned'
...
function importOrGetGameForPgn(pgnText, { originalFilename = 'custom_game.pgn', lichessGameId = null } = {}) {
  const existingId = lichessGameId ? findGameByLichessGameId(lichessGameId) : null;
  if (existingId) return getGame(existingId);
  return importCustomPgn(pgnText, { originalFilename, lichessGameId });
}
